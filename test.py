...
def _crawl_naver_keywords(text):
    ...
    # 한글 지원을 위해 앞에 unicode u를 붙혀준다.
        ...
        if( "지금" in text ) or ( "now" in text ) :
            ...
            for data in soup.find("div",class_="table_info weekly _weeklyWeather").find_all("span",class_="point_time morning"):
                data = data.find("span",class_="rain_rate").find("span",class_="num")
                if len(morning_r) >= 10 :
                    break
                morning_r.append(data.get_text().replace('\n',''))
            for data in soup.find("div",class_="table_info weekly _weeklyWeather").find_all("span",class_="point_time afternoon"):
                data = data.find("span",class_="rain_rate").find("span",class_="num")
            ...
            for i in range(0,len(afternoon_r)) :
                message = when[i] + " || 온도 - (" + morning_t[i]  + ")  | 강수확률 - ("+  morning_r[i] + "% / " + afternoon_r[i] + "% )"
